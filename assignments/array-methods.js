...
runners.map(function (runner) {
    allCaps.push(`${runner.first_name}`);
});
console.log(allCaps); 
...
// Problem 1
// forEach
//get a list of emails for each runner
let runnerEmail = [];
runners.forEach(function(runner) {
...
// Problem 2
// map
//divide donation number by id
let runnerProduct = [];
runners.map(function(runner) {
    runnerProduct.push(runner.donation / runner.id);
});
console.log(runnerProduct);

// Problem 3
// reduce
// add the id numbers
let runnerIdTotal = runners.reduce(function(sum, runner) {
    return sum + runner.id;
}, 0);
console.log(runnerIdTotal);